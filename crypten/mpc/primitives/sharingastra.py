...
def __replicated_secret_sharing_protocol(op, x, y, *args, **kwargs):
    ...
    if rank == 0:
        ...
        req0.wait()
        # print(torch.stack([torch.zeros_like(lambda_z1),lambda_z1,lambda_z2]))
        return torch.stack([lambda_z1,lambda_z2])
    ...
    # elif isinstance(x, AstraSharedTensor):
    #     z = AstraSharedTensor.from_shares(z, src=rank)
    #     z += AstraSharedTensor.PRZS(z.size(), device=z.device)
