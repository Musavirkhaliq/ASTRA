...
class AstraSharedTensor(object):
    """
    ...
    def __init__(
        ...
        seeds = AstraSharedTensor.astrashares(size, device=device).share
    ...
    def new(*args, **kwargs):
        """
        Creates a new AstraSharedTensor, passing all args and kwargs into the constructor.
        """
        return AstraSharedTensor(*args, **kwargs)
    ...
    def from_shares(share, precision=None, device=None):
        """Generate an AstraSharedTensor from a share from each party"""
        result = AstraSharedTensor(src=SENTINEL)
    ...
    def PRSS(*size, device=None):
        ...
        tensor = AstraSharedTensor.from_shares(share=share)
        return tensor
    ...
    def __repr__(self):
        return f"AstraSharedTensor({self.share})"

    def __bool__(self):
        """Override bool operator since encrypted tensors cannot evaluate"""
        raise RuntimeError("Cannot evaluate AstraSharedTensors to boolean values")
    ...
    def __setitem__(self, index, value):
        ...
        if isinstance(value, (int, float)) or is_tensor(value):
            value = AstraSharedTensor(value)
        assert isinstance(
            value, AstraSharedTensor
    ...
    def pad(self, pad, mode="constant", value=0):
        ...
        if isinstance(value, (int, float)):
        ...
        elif isinstance(value, AstraSharedTensor):
            assert (
                value.dim() == 0
            ...
            raise TypeError(
                "Cannot pad AstraSharedTensor with a %s value" % type(value)
            )
    ...
    def stack(tensors, *args, **kwargs):
        ...
        for i, tensor in enumerate(tensors):
            if is_tensor(tensor):
                tensors[i] = AstraSharedTensor(tensor)
            assert isinstance(
                tensors[i], AstraSharedTensor
            ), "Can't stack %s with AstraSharedTensor" % type(tensor)
    ...
    def reveal_batch(tensor_or_list, dst=None):
        ...
        if isinstance(tensor_or_list, AstraSharedTensor):
    ...
    def _arithmetic_function(self, y, op, inplace=False, *args, **kwargs):  # noqa:C901
        ...
        private = isinstance(y, AstraSharedTensor)
        if inplace:
            result = self
    ...
    def index_add_(self, dim, index, tensor):
        ...
        private = isinstance(tensor, AstraSharedTensor)
        if public:
    ...
    def scatter_add_(self, dim, index, other):
        ...
        private = isinstance(other, AstraSharedTensor)
        if public:
            if self.rank == 0:
    ...
    def where(self, condition, y):
        ...
        Args:
            condition (torch.bool or AstraSharedTensor): when True
                yield self, otherwise yield y.
            y (torch.tensor or AstraSharedTensor): values selected at
                indices where condition is False.

        Returns: AstraSharedTensor or torch.tensor
        """
    ...
    def scatter_(self, dim, index, src):
        ...
        """
        if is_tensor(src):
            src = AstraSharedTensor(src)
        assert isinstance(
            src, AstraSharedTensor
...
for func in regular.__all__:
    if not hasattr(AstraSharedTensor, func):
        setattr(AstraSharedTensor, func, getattr(regular, func))
